...
class TestFCM(unittest.TestCase):
    ...
    def test_evolve_once(self):
        ...
        # Calculate the expected output based on the initial state and weights
        expected_output_map = {
            # Calculated based on inputs and step function (>= 0.5)
            # stas: 0.4(HCP) + 0.4(inju) = 0.8 -> 1
            # ADP: 0 -> 0
            # PAgg: 0.95(A2) + 1(K) = 1.95 -> 1
            # K: -0.9(war) = -0.9 -> 0
            # Others are straightforward sums of weights from active nodes
            "HCP": 1, "stas": 1, "inju": 1, "HCF": 1, "ADP": 0, "PAgg": 1,
            "clop": 0, "A2": 0, "war": 0, "K": 0, "cox": 0, "aspi": 0
        }
        expected_vector = np.array([expected_output_map.get(node, 0) for node in node_order])
